...
function TargetAPE() {
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      MTDTarget = MTDTarget + parseFloat(doc.data().MTDTarget_2);
      MTDIssue = MTDIssue + parseFloat(doc.data().MTDIssue_2);
    });
...
function Achievement() {
  var str = "";
  var sRH = "";
  str += '<div class="btn-t33" style="margin-top:30px; background-color: #94a9b3;border: solid #94a9b3 1px;">หมวด % C2G+BT+CCC Achievement<br>'+xMonthDetail+'</div>';
  ...
  dbBBDRH.orderBy('Achieve_2','desc')
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      if(doc.data().MTDTarget_2<doc.data().MTDIssue_2) { 
        str += '<div class="progress2" style="float: left;width:60%;margin-top:6px;"><div class="bar4" style="width:'+ doc.data().Achieve_2 +'"></div></div>';
      } else {
        str += '<div class="progress2" style="float: left;width:60%;margin-top:6px;"><div class="bar2" style="width:'+ doc.data().Achieve_2 +'"></div></div>';
      }
      str += '<div class="bar_body1" onclick="OpenProfile(\''+ doc.id +'\')" style="width:15%;margin-left:10px;">'+ doc.data().Achieve_2 +'</div>';
      ...
      if(doc.data().EmpRH!=sRH) {
        ...
        .get().then((snapshot)=> {
          snapshot.forEach(doc=> {
            ...
            str1 += '<div class="progress2" style="float: left;width:60%;margin-top:6px;"><div class="bar2" style="width:'+ doc.data().Achieve_2 +'"></div></div>';
            str1 += '<div class="bar_body1">'+ doc.data().Achieve_2 +'</div>';
...
function ShowZone(RH) {
  var str = "";
  str += '<center><div class="btn-t4" style="margin-top:30px;margin-bottom: 14px;">หมวด % C2G+BT+CCC Achievement<br>'+xThisMonth+'<br>สำนักงานภาคธุรกิจสาขา สังกัด '+RH+'</div>';
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      if(doc.data().MTDTarget_2<doc.data().MTDIssue_2) {
        str += '<div class="progress2" style="float: left;width:46%;margin-top:6px;"><div class="bar4" style="width:'+ doc.data().Achieve_2 +'"></div></div>';
      } else {
        str += '<div class="progress2" style="float: left;width:46%;margin-top:6px;"><div class="bar3" style="width:'+ doc.data().Achieve_2 +'"></div></div>';
      }
      str += '<div class="bar_body">'+ doc.data().Achieve_2 +'</div>';
      str += '</div><div class="clr"></div></div>';
    });
